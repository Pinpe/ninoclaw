...
def call_api(user: str, system: str = '空') -> str:
    '''
    ...
    while True:
        try:
            ...
            response = client.chat.completions.create(
                model    = config['model'],
                stream   = False,
                extra_body = ({"thinking": {"type":"enabled"}} if config['think']==True 
                    else {"thinking": {"type":"disabled"}}) if config['think'] is not None 
                    else None,
                messages = [
            ...
            if api_retry_num < config['api_retry_num']:
                time.sleep(config['api_retry_sleep'])
                api_retry_num += 1
            else:
                return str(f'API出现错误：{err}')
"""
...
def command_exec(input: str, path: str) -> str:
    ...
    '''
    global config
    time.sleep(config['cmd_sleep'])  # 这里加个阻塞，调用太快可能会被排队
    ...
    try:
        output =  subprocess.run(
            ...
            text           = True,
            cwd            = path,
            timeout        = config['cmd_time_out'],  # 超时会抛出TimeoutExpired，下面也有捕获
        ...
        return f'执行命令超时：{config['cmd_time_out']}秒上限'
    except Exception as err:
...
def ping(url: str) -> bool:
    ...
    try:
        requests.get(
            url,
            timeout = config['connect_check_time_out']
        ).status_code
        return True
    except Exception:
        return False
