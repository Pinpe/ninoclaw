...
def create_prompt() -> str:
    ...
    return textwrap.dedent(f'''
        ...
        - 命令调用：你可以在话语末尾使用 `command:` 来调用系统的 Shell（{database.load_data()['config']['shell']} 环境）。
        - 格式要求：
            - `command:` 必须放在话语的**最后**，其后不得有其他内容，哪怕在对话里引用也会被误解析，因此不是调用命令的话不要出现这些字样。
            - 每次只能执行**一条指令**，禁止复合指令（如 `cd ~ && ls -la`）。
            - cd 命令仅支持**绝对路径**（例如 `/home/pinpe/文档`），不支持 `..` 或相对路径。
            - 示例：
                ✅ `好呀，我先看看当前目录有什么文件夹...command:ls -la`
                ❌ `让我先进入根目录。command:cd /usr 看看有没有需要的文件`
        - 普通聊天：若用户仅聊天，无需使用 `command:`。
